...
$sql = "SELECT c.id,b.title,b.author,b.publisher,b.edition,b.barcode,c.date_reserv,c.localisation,c.reserver";
$sql.=" FROM book b inner join concession c on b.id = c.idBook";
$sql.=" WHERE c.reserver != 0 and c.reserver != -3";
...
if (!empty($requestData['search']['value'])) {   //name
    ...
    $sql.=" OR localisation '".$requestData['search']['value']."%' ";
    $sql.=" OR edition LIKE '".$requestData['search']['value']."%' ";
    $sql.=" OR date_reserv LIKE '".$requestData['search']['value']."%' ";
